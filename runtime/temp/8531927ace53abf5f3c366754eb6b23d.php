...
<style>
	...
	<div class="cf">
		<a class="btn" href="<?php echo url('noticeAdd'); ?>">新 增</a>
	</div>
			...
			<tbody>
				...
				<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$notice): $mod = ($i % 2 );++$i;?>
					<tr>
						...
						<td>
							<a title="编辑" href="<?php echo url('noticeUpdate?id='.$notice['id']); ?>">编辑</a>
							<a class="confirm ajax-get" title="删除" href="<?php echo url('noticeDel?id='.$notice['id']); ?>">删除</a>
						</td>
					</tr>
				<?php endforeach; endif; else: echo "" ;endif; ?>
