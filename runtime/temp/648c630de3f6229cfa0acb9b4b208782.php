<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:72:"D:\www\thinkphp\public/../application/admin/view/default/model\edit.html";i:1496373782;s:73:"D:\www\thinkphp\public/../application/admin/view/default/public\base.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
		...
		<div class="tab-content">
			...
			<form id="form" action="<?php echo url('update'); ?>" method="post" class="form-horizontal doc-modal-form">
				...
				<div id="tab1" class="tab-pane in tab1">
					...
					<div class="form-item cf">
						<label class="item-label">模型类型<span class="check-tips">（选择继承模型）</span></label>
						<div class="controls">
							<select name="extend">
				...
				<div id="tab2" class="tab-pane tab2">
					<div class="form-item cf">
						...
						<div class="form-item cf edit_sort edit_sort_l form_field_sort">
							<span>字段列表 		[ <a href="<?php echo url('Attribute/add?model_id='.$info['id']); ?>" target="_balnk">新增</a>
							<a href="<?php echo url('Attribute/index?model_id='.$info['id']); ?>" target="_balnk">管理</a> ] </span>
							<ul class="dragsort"> 
								<?php if(is_array($fields) || $fields instanceof \think\Collection || $fields instanceof \think\Paginator): if( count($fields)==0 ) : echo "" ;else: foreach($fields as $k=>$field): ?>
										<li >
											<em >
											<input class="ids" type="checkbox" name="attribute_list[]" value="<?php echo $field['id']; ?>"/> <?php echo $field['title']; ?> [<?php echo $field['name']; ?>]</em>
										</li>
								<?php endforeach; endif; else: echo "" ;endif; ?>
							</ul>
						</div>
					...
					<?php $_result=parse_field_attr($info['field_group']);if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
						<div class="form-item cf edit_sort edit_sort_l form_field_sort">
							<span><?php echo $vo; ?></span>
							<ul class="dragsort needdragsort" data-group="<?php echo $key; ?>">
								<?php if(is_array($fields) || $fields instanceof \think\Collection || $fields instanceof \think\Paginator): if( count($fields)==0 ) : echo "" ;else: foreach($fields as $k=>$field): isset($field['group']) ? $field['group'] : $field['group'] ='1' ;if((($field['group'] == $key) or($i == 1 and !isset($field['group']))) and ($field['is_show'] == 1)): ?>
										<li class="getSort">
